...
public class VisitHistory extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (!request.getParameter("user_id").equals((String) session.getAttribute("user"))) {
		...
		if (userId != null) {
			visited = connection.getVisitedRestaurants(userId);
			for (String businessId : visited) {
				result.put(connection.getRestaurantsById(businessId, true));
			}
			RpcParser.writeOutput(response, result);
		} else {
			try {
				RpcParser.writeOutput(response, new JSONObject().put("status", "Illegal Parameter"));
	...
	protected void doDelete(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			if (!((String) input.get("user_id")).equals((String) session.getAttribute("user"))) {
				response.setStatus(403);
				return;
			}
